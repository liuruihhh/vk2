...
void VkModel::drawFrame(float delta, float time)
{
	updateUniformBuffer(delta, time);
	recordCommandBuffer();
}

void VkModel::updateUniformBuffer(float delta, float time) {
	UniformBufferObject ubo{};
	ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(.1f), glm::vec3(0.0f, 0.0f, 1.0f));
